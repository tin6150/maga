...
I guess include in head then :)    #Q4Gene
...
Something about DCC vs ??  The doctors please help here! -Tin :)
...
What type of variable(s) are you investigating (continuous, categorical, ordinal, etc)?
	...
	Some are categorical, eg died, groups, sex, los.
	others are continuous, eg ga, bw
	
	Because the way how R imported the source data file, most of the data were read as string (part of the problem maybe spaces).  Overall, mapping function is used to create extra columns that contain data that R correctly parse into categories.  eg:
...
## but these may not show poisson distribution stuff
## 
...
•
What theoretical distribution that we have talked about would potentially be appropriate to use with
