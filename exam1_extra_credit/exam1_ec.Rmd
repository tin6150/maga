...
+ 1 pt
Correct: Shape = skewed right; Center = mean around 6-7 days or median around 5-6 days; Spread = from 1 day to 15 days
...
prep_data = rot_csv %>% mutate( gfr = gpa, prep_time = xtime )
str(prep_data)


val_mean_prep_over_50 = prep_data %>% filter( age >= 20) %>% mutate( gfr_ratio = gfr/prep_time) %>% summarize( mean_gfr_ratio = mean(gfr_ratio) )

mean_prep_over_50 = prep_data %>% filter( age >= 20 ) %>%   mutate( gfr_ratio = gfr/prep_time, mean_gfr_ratio = val_mean_prep_over_50 )

val_mean_prep_over_50
mean_prep_over_50
...
Internal validity was accomplied by:
The distribution of gpa  was NOT pre-determined by the researchers.  
Students are randomly selected, regarless of gender, age, major of studies.  
